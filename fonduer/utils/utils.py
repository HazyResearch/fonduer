...
def get_ORM_instance(ORM_class, session, instance):
    """
    Given an ORM class and *either an instance of this class, or the name
    attribute of an instance of this class*, return the instance.
    """
...
def matrix_overlaps(L):
    ...
    return np.ravel(
        np.where(L_nonzero.sum(axis=1) > 1, 1, 0).T * L_nonzero / float(L.shape[0])
    )
...
def matrix_conflicts(L):
    ...
    if not (
        sparse.isspmatrix_csc(B) or sparse.isspmatrix_lil(B) or sparse.isspmatrix_csr(B)
    ):
        raise ValueError("Only supports CSR/CSC and LIL matrices")
