...
from .models import Candidate, Sentence, TemporarySpan
...
class CandidateExtractor(UDFRunner):
    ...
    :param candidate_filter: an optional function for filtering out candidates
                             which returns a Boolean expressing whether or
                             not the candidate should be instantiated.
...
class CandidateExtractorUDF(UDF):
    def __init__(self, candidate_class, cspaces, matchers, candidate_filter,
        ...
        self.candidate_spaces = cspaces if isinstance(
            cspaces, (list, tuple)) else [cspaces]
        self.matchers = matchers if isinstance(matchers,
                                               (list, tuple)) else [matchers]
    ...
    def apply(self, context, clear, split, **kwargs):
        ...
        for i in range(self.arity):
            ...
            for tc in self.matchers[i].apply(
                    self.candidate_spaces[i].apply(context)):
        ...
        for args in product(*[
            ...
            if self.arity == 2:
                ...
                if not self.self_relations and a == b:
                    ...
                    continue
                elif not self.symmetric_relations and (
                    (b, a) in extracted or
                    (self.matchers[0] == self.matchers[1]
...
class PretaggedCandidateExtractor(UDFRunner):
    ...
    def apply(self, xs, split=0, **kwargs):
        super(PretaggedCandidateExtractor, self).apply(
            xs, split=split, **kwargs)
