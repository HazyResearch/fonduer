...
if 'CI' not in os.environ:
    try:
        ...
        logger.warning(
            "nltk not installed- some default functionality may be absent.")
...
class Matcher(object):
    ...
    def f(self, c):
        ...
        if len(self.children) == 0:
            ...
            raise Exception("%s does not support more than one child Matcher" %
                            self.__name__)
...
class DictionaryMatch(NgramMatcher):
    ...
    def init(self):
        ...
        try:
            ...
            raise Exception(
                "Please supply a dictionary (list of phrases) d as d=d.")
...
class Concat(NgramMatcher):
    ...
    def f(self, c):
        if len(self.children) != 2:
            raise ValueError(
                "Concat takes two child Matcher objects as arguments.")
        ...
        for wsplit in range(c.get_word_start() + 1, c.get_word_end() + 1):
            csplit = c.word_to_char_index(
                wsplit
            ) - c.char_start  # NOTE the switch to **candidate-relative** char index
            ...
            if self.ignore_sep or c.get_span()[csplit - 1] == self.sep:
                ...
                if self.children[0].f(c1) and self.children[1].f(c2):
                    return True
                if self.permutations and self.children[1].f(
                        c1) and self.children[0].f(c2):
                    return True
        return False
...
class SlotFillMatch(NgramMatcher):
    ...
    def init(self):
        ...
        try:
            ...
            raise Exception(
                "Please supply a slot-fill pattern p as pattern=p.")
...
class RegexMatch(NgramMatcher):
    ...
    def init(self):
        try:
            ...
            raise Exception(
                "Please supply a regular expression string r as rgx=r.")
        self.ignore_case = self.opts.get('ignore_case', True)
        self.attrib = self.opts.get('attrib', WORDS)
        self.sep = self.opts.get('sep', " ")
...
class RegexMatchEach(RegexMatch):
    ...
    def _f(self, c):
        ...
        return True if tokens and all(
            [self.r.match(t) is not None for t in tokens]) else False
