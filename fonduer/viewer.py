...
class Viewer(widgets.DOMWidget):
    ...
    def __init__(self,
        ...
        :param session: The Session for the database backend
        ...
        self.candidates = sorted(
            list(candidates), key=lambda c: c[0].char_start)
        self.contexts = list(
        ...
        for i, candidate in enumerate(self.candidates):
            ...
            existing_annotation = self.session.query(GoldLabel).filter(
                GoldLabel.key == self.annotator).filter(
            ...
            if existing_annotation is not None:
                ...
                existing_annotation_stable = self.session.query(
                    StableLabel).filter(
                ...
                if existing_annotation_stable is None:
                    context_stable_ids = '~~'.join(
                        [c.stable_id for c in candidate.get_contexts()])
                    existing_annotation_stable = StableLabel(
    ...
    def handle_label_event(self, _, content, buffers):
        ...
        if content.get('event', '') == 'set_label':
            ...
            if value is True:
                ...
                raise ValueError(
                    'Unexpected label returned from widget: ' + str(value) +
            ...
            # Otherwise, create a AnnotatorLabel *and a StableLabel*
                ...
                self.annotations[cid] = GoldLabel(
                    key=self.annotator, candidate=candidate, value=value)
                self.session.add(self.annotations[cid])

                # Create StableLabel
                context_stable_ids = '~~'.join(
                    [c.stable_id for c in candidate.get_contexts()])
                self.annotations_stable[cid] = StableLabel(
                    context_stable_ids=context_stable_ids,
                    annotator_name=self.annotator.name,
                    value=value,
                    split=candidate.split)
...
class SentenceNgramViewer(Viewer):
    ...
    def __init__(self,
                 ...
                 height=225,
                 annotator_name=None):
        super(SentenceNgramViewer, self).__init__(
            candidates,
            session,
            gold=gold,
    ...
    def _tag_context(self, sentence, candidates, gold):
        ...
        splits = sorted(
            list(
                set([b.char_start
                     for b in both] + [b.char_end + 1
                                       for b in both] + [0, len(s)])))
        ...
        for i in range(len(splits) - 1):
            ...
            try:
                ...
                cids0 = [
                    self.candidates.index(c) for c in candidates
                    if self._is_subspan(start, end, c[0])
                ]
                cids0 += ['%s-0' % cid for cid in cids0]
                cids1 = [
                    self.candidates.index(c) for c in candidates
                    if self._is_subspan(start, end, c[1])
                ]
                ...
                # Handle gold...
                gcids = [
                    self.gold.index(g) for g in gold
                    if self._is_subspan(start, end, g[0])
                    or self._is_subspan(start, end, g[1])
                ]
            except Exception as e:
                cids = [
                    self.candidates.index(c) for c in candidates
